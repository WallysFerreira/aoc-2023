defmodule Day9Part2 do
  ...
  def get_previous_values(list) do
    old_list =
    ...
    |> Enum.reduce(old_list, fn current, updated_lists ->
      ...
      new_list =
        current_list
        |> List.insert_at(0, List.first(current_list) - List.first(dif_list))
  ...
  def sum_previous_values(lists) do
    lists
    |> Enum.map(&get_previous_values(&1))
